...
def main() -> None:
    ...
    male_strs = [s.strip() for s in args.male_strings.split(",") if s.strip()]
    female_strs = [s.strip() for s in args.female_strings.split(",") if s.strip()]
    ...
    print(f"[W] learned w over k={args.k} dims, ||w||={float(w.norm().item()):.6g}, oriented so ' he' > ' she'.")
    ...
    def print_table(title: str, res: Dict[str, Any]) -> None:
        print(f"\n=== {title} ===")
        print("scale\tflip_he%\tflip_she%\tKL_mean")
        for s in sigma_scales:
            k = str(float(s))
            r = res[k]
            print(f"{float(s):g}\t{r['flip_he_pct']:.2f}\t{r['flip_she_pct']:.2f}\t{r['kl_mean']:.4g}")
