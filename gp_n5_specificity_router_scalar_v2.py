...
def identity_z_head_sanity(
    ...
    # Cache corrupt hook_z for this layer WITHOUT quantization.
    # IMPORTANT: do NOT round to float16 here. Even tiny rounding noise can change a(x)
    # and falsely fail the identity check.
    hook_name_z = f"blocks.{layer_check}.attn.hook_z"
    _oz, cache_z = model.run_with_cache(tokens_corr, names_filter=[hook_name_z], stop_at_layer=layer_check + 1)
    z_corr_full = cache_z[hook_name_z].detach()  # (B, S_pad, H, d_head) on-device, full precision
