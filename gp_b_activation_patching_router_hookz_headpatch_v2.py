...
def identity_z_head_sanity(
    ...
    # Identity sanity for head-level patching:
    # - Cache corrupt hook_z (FULL tensor) for layer_check in native dtype (no fp16 downcast),
    #   then re-run corrupt with a hook that overwrites that head's z for all positions p<=t* with cached values.
    # - Compare resulting a(x) with the unpatched corrupt a(x).
    #
    # Why: If you cache to fp16 and patch an entire head across all positions, you can get ~1e-5..1e-4 drift
    # purely from quantization. This check aims to validate indexing / masking, not cache precision.

    print(f"[SANITY-ID-Z-HEAD] identity patch on hook_z at (L={layer_check}, H={head_check}) across all p<=t*")

    # Tolerance depends on model dtype (fp16 is noisier).
    tol = 1e-4 if getattr(model.cfg, "dtype", None) in (torch.float16, torch.bfloat16) else 1e-5
    ...
    for i, j in batches_idx(len(pairs_sel), batch_size):
        ...
        # Baseline corrupt a(x)
        a_corr, _ = run_and_get_ai(model, tokens_corr, last_idx, layer_star, head_star, u_vec)

        # Cache corrupt z FULL for this layer in native dtype ON DEVICE (no fp16 CPU downcast)
        _o_z, cache_z = model.run_with_cache(
            tokens_corr,
            names_filter=[hook_name],
            stop_at_layer=layer_check + 1,
        )
        z_corr_full = cache_z[hook_name].detach()  # (B,S_pad,H,Dh), device, native dtype

        def hook_fn(z: torch.Tensor, hook=None):
            z2 = z.clone()
            _B, S_pad, _H, _Dh = z2.shape
            mask = (torch.arange(S_pad, device=z2.device)[None, :] <= last_idx[:, None])  # (B,S_pad)
            mask3 = mask[:, :, None]
            z2[:, :, head_check, :] = torch.where(
                mask3,
                z_corr_full[:, :, head_check, :].to(z2.dtype),
                z2[:, :, head_check, :],
            )
            return z2
    ...
    print(f"  max|a_id - a_corr| = {max_abs:.3e} (tol={tol:.1e})")
    if max_abs > tol:
        raise RuntimeError(
            "SANITY-ID-Z-HEAD FAILED: identity head patch changed a(x) more than tolerance. "
            "If this is only slightly above tol, it may be GPU non-determinism; try rerun with the same seed."
        )
