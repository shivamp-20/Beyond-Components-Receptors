...
def _find_free_port() -> int:
    # Pick a free port for TCPStore rendezvous (single-node).
    # This avoids 'address already in use' when multiple notebooks run.
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        s.bind(("127.0.0.1", 0))
        return int(s.getsockname()[1])


def _ddp_setup(rank: int, world_size: int, backend: str, port: int, timeout_sec: int) -> None:
    # For single-node multi-GPU, use env:// rendezvous.
...
def _rank0_print(rank: int, *args, **kwargs) -> None:
    if _is_rank0(rank):
        print(*args, **kwargs)
...
def _ddp_broadcast_int(rank: int, value: int) -> int:
    t = torch.tensor([int(value)], device="cuda" if torch.cuda.is_available() else "cpu", dtype=torch.int64)
    if dist.is_available() and dist.is_initialized():
        dist.broadcast(t, src=0)
    return int(t.item())


def _ddp_broadcast_bool(rank: int, value: bool) -> bool:
    v = 1 if value else 0
    v = _ddp_broadcast_int(rank, v)
    return bool(v)
...
def _ddp_shard_rows(
    ...
    seed: int,
    shuffle: bool,
    pad_to_even_steps: bool = True,
) -> List[int]:
    """Return a list of row indices assigned to this rank.

    IMPORTANT: During *training* under DDP, all ranks must execute the same number of
    backward steps. We enforce this by (optionally) padding indices so that:
      total_size % (world_size * per_rank_batch) == 0
    and each rank gets exactly total_size/world_size samples.

    For evaluation we can skip padding (pad_to_even_steps=False) and instead
    reduce (sum,count) across ranks.
    """
    n = len(rows)
    idx = list(range(n))
    ...
    if not pad_to_even_steps:
        # Each rank gets roughly n/world_size samples
        return idx[rank::world_size]

    # Pad so that each rank sees the same number of *batches*.
    global_batch = per_rank_batch * world_size
    if global_batch <= 0:
        raise ValueError("Invalid per-rank batch size (<=0).")

    # Round up to the next multiple of global_batch
    total_size = ((n + global_batch - 1) // global_batch) * global_batch
    if total_size == 0:
        return []

    if total_size > n:
        # Repeat from the start to pad (doesn't change logic; just repeats a few samples)
        idx = idx + idx[: (total_size - n)]

    # Now shard
    return idx[rank:total_size:world_size]
...
def _ddp_batch_iter(
    ...
    seed: int,
    shuffle: bool,
    pad_to_even_steps: bool,
):
    indices = _ddp_shard_rows(
        rows=rows,
        ...
        seed=seed,
        shuffle=shuffle,
        pad_to_even_steps=pad_to_even_steps,
    )
    for i in range(0, len(indices), per_rank_batch):
        batch_idx = indices[i:i + per_rank_batch]
        if len(batch_idx) == 0:
            continue
        if len(batch_idx) < per_rank_batch and pad_to_even_steps:
            # Should never happen when pad_to_even_steps=True (we pad indices).
            continue
...
def _parse_args():
    ...
    # IMPORTANT: batch_size is the *GLOBAL* batch size (across all GPUs).
    # If you used batch_size=64 on 1 GPU, keep it 64 on 2 GPUs to preserve training dynamics.
    # The script will automatically use per_rank_batch = batch_size / world_size.
    ...
    # ---- DDP / multi-GPU ----
    # We keep your launch command the same (`python ...`). If 2 GPUs are available,
    # default behavior (ddp_mode=auto) will spawn 1 process per GPU (DDP) automatically.
    ap.add_argument("--ddp_mode", type=str, default="auto", choices=["auto", "off", "on"],
                    help="auto: use DDP when >1 GPU is visible; off: force single GPU; on: force DDP (if possible).")
    ap.add_argument("--ddp_backend", type=str, default="nccl", choices=["nccl", "gloo"],
                    help="nccl recommended for CUDA; gloo is CPU-friendly fallback.")
    ap.add_argument("--ddp_port", type=int, default=29500,
                    help="Rendezvous port. Use 0 to auto-pick a free port.")
    ap.add_argument("--ddp_timeout", type=int, default=1800,
                    help="Timeout (seconds) for DDP init/barriers. Increase if your notebook is slow.")
    args = ap.parse_args()
    return args
...
def _worker(rank: int, world_size: int, args) -> None:
    ...
    # Note: for single-node multi-GPU, rank maps directly to CUDA device index.
    if args.device != "cpu" and torch.cuda.is_available():
        torch.cuda.set_device(rank)
        device = f"cuda:{rank}"
        torch_device_for_reduce = "cuda"
    else:
        device = "cpu"
        torch_device_for_reduce = "cpu"

    if world_size > 1:
        _ddp_setup(rank, world_size, backend=args.ddp_backend, port=args.ddp_port, timeout_sec=args.ddp_timeout)

    # Make tokenizers quieter in multi-proc environments
    os.environ.setdefault("TOKENIZERS_PARALLELISM", "false")

    # Seeds: keep deterministic shuffling per epoch via our own RNG; this just makes
    # any incidental randomness stable per-rank.
    ...
    # ---- SVD cache (rank0 builds, others wait) ----
    need_build = args.rebuild_svd or (not os.path.exists(svd_path))
    if world_size > 1:
        need_build = _ddp_broadcast_bool(rank, need_build)
    ...
    # ---- DDP batch sizing ----
    # args.batch_size is global batch size.
    if world_size > 1:
        if args.batch_size % world_size != 0:
            raise ValueError(
                f"--batch_size ({args.batch_size}) must be divisible by world_size ({world_size}). "
                f"Tip: keep global batch_size the same as your 1-GPU run (e.g. 64), so per-GPU becomes 32."
            )
    per_rank_batch = args.batch_size // world_size
    if per_rank_batch <= 0:
        raise ValueError("Per-rank batch size became <= 0. Increase --batch_size.")

    if _is_rank0(rank):
        if world_size > 1:
            print(f"[DDP] world_size={world_size} | global_batch={args.batch_size} => per_gpu_batch={per_rank_batch}")
        else:
            print(f"[SINGLE] batch={args.batch_size}")

    # ---- trainable masks ----
    masks = MaskBank(qk=qk, ov=ov, mlp_in=mlp_in, mlp_out=mlp_out).to(device)
    if world_size > 1:
        # DDP will all-reduce gradients for MaskBank params.
        # Note: we use the params directly in the loss computation (no explicit masks.forward()).
        masks_ddp = DDP(masks, device_ids=[rank] if device != "cpu" else None, broadcast_buffers=False)
        masks_mod = masks_ddp.module
        opt = torch.optim.AdamW(masks_ddp.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    else:
        masks_ddp = None
        masks_mod = masks
    ...
    for epoch in range(1, args.max_epochs + 1):
        ...
        # IMPORTANT: pad_to_even_steps=True so every rank does same number of backward steps.
        for batch in _ddp_batch_iter(train_rows, per_rank_batch, rank, world_size, epoch, args.seed, shuffle=True, pad_to_even_steps=True):
            ...
            l1 = masks_mod.l1_sum(mode=args.l1_mode)
        ...
        # ---- reduce training stats across ranks ----
        if world_size > 1:
            tl = torch.tensor(total_loss, device=torch_device_for_reduce, dtype=torch.float64)
            tk = torch.tensor(total_kl, device=torch_device_for_reduce, dtype=torch.float64)
            t1 = torch.tensor(total_l1, device=torch_device_for_reduce, dtype=torch.float64)
            tb = torch.tensor(total_batches, device=torch_device_for_reduce, dtype=torch.float64)
            _ddp_all_reduce_sum(tl); _ddp_all_reduce_sum(tk); _ddp_all_reduce_sum(t1); _ddp_all_reduce_sum(tb)
            total_loss = float(tl.item())
            total_kl = float(tk.item())
            total_l1 = float(t1.item())
            total_batches = int(tb.item())

        # ---- validation (no backward; we can shard without padding and reduce sum/count) ----
        masks_mod.eval()
        with torch.no_grad():
            val_kl_sum = 0.0
            val_count = 0

            for batch in _ddp_batch_iter(val_rows, per_rank_batch, rank, world_size, epoch, args.seed, shuffle=False, pad_to_even_steps=False):
                clean_prompts = [r["prefix"] for r in batch]
                corr_prompts = [r["corr_prefix"] for r in batch]
                tokens_clean, last_idx_clean, tokens_corr, _ = tokenize_pair_batch(
                    tokenizer, clean_prompts, corr_prompts, device
                ...
                val_kl_sum += float(kl.cpu().item()) * float(tokens_clean.size(0))
                val_count += int(tokens_clean.size(0))

            if world_size > 1:
                vs = torch.tensor(val_kl_sum, device=torch_device_for_reduce, dtype=torch.float64)
                vc = torch.tensor(val_count, device=torch_device_for_reduce, dtype=torch.float64)
                _ddp_all_reduce_sum(vs); _ddp_all_reduce_sum(vc)
                val_kl_sum = float(vs.item())
                val_count = int(vc.item())

            val_kl = float(val_kl_sum / max(1, val_count))

        # ---- metrics (computed on rank0; masks are synced) ----
        spars = compute_sparsities(masks_mod, tau=args.tau, rank_total_ov=rank_total_ov)
        ...
        with torch.no_grad():
            l1_now_mean_per_vector = float(masks_mod.l1_sum(mode="mean_per_vector").detach().cpu().item())
            l1_now_sum_all = float(masks_mod.l1_sum(mode="sum_all").detach().cpu().item())
        ...
        masks_all = collect_mask_values(masks_mod, which='all')
        masks_ov = collect_mask_values(masks_mod, which='ov')
        ...
        # ---- early stop (rank0 decides; broadcast to all ranks) ----
        stop_now = False
        if _is_rank0(rank):
            ...
            if not stop_now:
                if val_kl < best_val:
                    best_val = val_kl
                    bad_epochs = 0
                    save_masks(masks_path, masks_mod)
        ...
        if world_size > 1:
            stop_now = _ddp_broadcast_bool(rank, stop_now)

        if stop_now:
            break

    # ---- final save + dump (rank0 only) ----
    if _is_rank0(rank):
        save_masks(masks_path, masks_mod)
        with open(summary_path, "w", encoding="utf-8") as f:
            json.dump(last_summary, f, indent=2)
...
def main():
    ...
    # Decide world_size
    n_gpus = torch.cuda.device_count() if (args.device != "cpu" and torch.cuda.is_available()) else 0

    use_ddp = False
    if args.ddp_mode == "off":
        use_ddp = False
    elif args.ddp_mode == "on":
        use_ddp = (n_gpus >= 2)
        if not use_ddp:
            print("[WARN] --ddp_mode=on but <2 GPUs available; running single-process.")
    else:  # auto
        use_ddp = (n_gpus >= 2)
    ...
    if use_ddp:
        # Pick a port if needed
        if args.ddp_port == 0:
            args.ddp_port = _find_free_port()

        # Safer multiprocessing start method for CUDA
        try:
            mp.set_start_method("spawn", force=True)
        except RuntimeError:
            pass

        print(f"[LAUNCH] DDP spawn: world_size={world_size}, backend={args.ddp_backend}, port={args.ddp_port}")
