...
def main():
    ...
    ap.add_argument(
        ...
        help=(
            "Optional zero-centering to remove constant offsets. "
            "'none': no centering. "
            ...
            "Use this if you suspect receptors have non-zero baseline activation."
        ),
    )
    args = ap.parse_args()
...
# -------------------------
# Motivation: your score s(l) can have a constant offset because receptor activations are not zero-centered.
# This does NOT affect AUC (ranking), but it can make acc@0 meaningless. We optionally remove the offset.
polarity_cpu = polarity.detach().cpu()  # needed for (re)computing s in 'acts' mode

center_mode = str(args.center_mode).lower()
all_s_raw = all_s.clone()
all_a_raw = all_a.clone()

mean_s_layer = all_s.mean(dim=0)                # (L,)
mean_a_layer = all_a.mean(dim=0)                # (L,3)

if center_mode == "none":
    pass
elif center_mode == "score":
    # s(l) <- s(l) - mean_{examples}(s(l))
    all_s = all_s - mean_s_layer.view(1, -1)
elif center_mode == "acts":
    # a(l) <- a(l) - mean_{examples}(a(l)); then recompute s(l) = v^T a(l)
    all_a = all_a - mean_a_layer.view(1, mean_a_layer.shape[0], 3)
    # recompute s from centered activations
    all_s = torch.einsum("nlk,k->nl", all_a, polarity_cpu.float())
else:
    raise ValueError(f"Unknown --center_mode {args.center_mode!r}. Expected one of: none, score, acts")

# quick diagnostic: mean across prompts should now be ~0 for centered s
mean_s_after = all_s.mean(dim=0)
if center_mode != "none":
    print(
        f"\n[CENTER] center_mode={center_mode} | "
        f"mean_s before (L11)={float(mean_s_layer[-1]):+.4g} after={float(mean_s_after[-1]):+.4g} (should be ~0)"
    )
    # -------------------------
    # Summaries per layer
    # -------------------------
    ...
    summary = {
        ...
        "mean_s_layer_raw": [float(x) for x in mean_s_layer.tolist()],
        "mean_a_layer_raw": [[float(y) for y in row] for row in mean_a_layer.tolist()],
...
# include raw + centering stats so you can compare later without re-running
payload["all_s_raw"] = all_s_raw
payload["all_a_raw"] = all_a_raw
payload["mean_s_layer_raw"] = mean_s_layer
payload["mean_a_layer_raw"] = mean_a_layer
...
if center_mode != "none":
    # compute raw RCS means per layer (based on raw activations)
    rcs_m_raw = []
    rcs_f_raw = []
    for l in range(n_layers):
