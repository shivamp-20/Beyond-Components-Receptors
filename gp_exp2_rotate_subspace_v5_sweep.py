...
def main() -> None:
    ...
    ap.add_argument("--run_name", type=str, default="",
                    help="Optional tag appended to output subdir name so sweeps do not overwrite exp2_results.json")
    ...
    run_tag = (args.run_name or "").strip()
    if run_tag:
        # sanitize for filesystem
        run_tag = run_tag.replace(" ", "_").replace("/", "_").replace("\\", "_")
    exp_name = f"L{args.layer}_H{args.head}_k{args.k}" + (f"__{run_tag}" if run_tag else "")
    exp_dir = out_dir / "exp2_rotate" / exp_name
