...
def sign_eps(x: np.ndarray, eps: float = 1e-12) -> np.ndarray:
    """Sign with an epsilon dead-zone (NumPy).
    ...
    We use this for sign-agreement curves so that near-zero projections
    (numerical noise / true ambiguity) don't cause spurious sign flips.
    """
    x = np.asarray(x)
...
def main():
    ...
    # Labels for the 13 residual snapshots (same length/order as hook_names).
    # index 0 = embedding snapshot before any transformer block (label -1)
    # index (l+1) = post-block-l residual (label l)
    layer_labels = [-1] + list(range(model.cfg.n_layers))
    ...
    # Ensure label and polarity tensors are on the same device as g
    y_t = ys.to(device=g.device, dtype=torch.float32)  # (N,)
    polarity_t = polarity.to(device=g.device, dtype=torch.float32)  # (3,)
    signed_margin_g = (
